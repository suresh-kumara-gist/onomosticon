...
class FilterOnomasticon extends FilterBase {
  ...
  public function replaceTerms($text) {
    ...
    $orientation_class = ' onomasticon-orientation-' . $orientation;
    // Set the cursor.
    $cursor_class = '';
    ...
    if ($implement == 'extra_element') {
      $ins_before = '<' . $tag . ' class="onomasticon onomasticon-extra-element' . $cursor_class . $orientation_class . '" tabindex="0" title="';
      $ins_between = '"><button disabled="disabled" class="onomasticon-term-description">';
      $ins_after = '</button></' . $tag . '>';
      $order = 1; // Term name first, description second
    }
    elseif ($implement == 'attr_title') {
      $ins_before = '<' . $tag . ' class="onomasticon' . $cursor_class . '" title="';
      $ins_between = '">';
      $ins_after = '</' . $tag . '>';
      $order = 2; // Term description first, name second
    }
    ...
    foreach ($this->getTaxonomyTerms() as $term) {
      ...
      $text = preg_replace("/\b" . $needle . "\b/", $placeholder, $text, $preg_limit);
    }
